...
kcal = TDEE + modeAdj(cut −500 / maintain 0 / bulk +500) + loggedWorkoutKcal
       + adaptive tdeeAdj (capped at ±150, needs ≥8 weigh-ins)
```
...
| M2 | **Adaptive TDEE can't rescue it** — capped at **±150** ([app.jsx:392](app.jsx#L392)), needs **≥8 weigh-ins** to start ([app.jsx:366](app.jsx#L366)). | The one mechanism that could learn the true TDEE is too small (±150 can't close a 500 gap) and too slow. |
...
>
> **Believability-check results (day-one seed, male; Fuel Log Katch-McArdle vs MFP Mifflin × textbook factor):**
...
|---|---|---|---|---|---|
| Founder — 98.5 kg / 30% BF, sedentary | 69 | 2,231 | 1,731 | 25.1 | raise to 2,070 → **deficit capped at 161 kcal** |
...
### 5.4 The auto-lowering fix — why the correction is one-directional (2026-08-09)

Decided by the founder after the file-04 rewrite. Spec:
`features/energy-safety/04-adaptive-tdee-guardrails.feature`.

This is the mechanism that caused the harm the whole workstream answers. The weekly
calibration corrects the app's guess at what you burn against the scale — and it was
**symmetric**, moving your target down for a disappointing week exactly as readily as it
moved it up for a good one. Those two directions carry very different costs. Guessing high
costs some progress. Guessing low walks a dieter toward under-eating in 25 kcal steps while
telling them it is the correct thing to do.

**The rule: the app only lowers its estimate when you are not cutting.** While the
prescribed target sits below maintenance, a disappointing scale never moves the number
down — not on a gain, not on a stall. In Maintain or Bulk it does, because there the
evidence is clean: eating at maintenance and still not losing genuinely does mean a lower
burn. Raising is never damped in any mode; finding out you burn *more* is good news and
should arrive as fast as the evidence does.

Whether the week counted as a cut reads the **declared daily mode** from the history
snapshots — file 02's signal, by majority over the measured week — so it holds up for a
patchy logger rather than depending on food logs.

The obvious objection is that the app can then never correct an over-estimate during a
cut, and people cut most of the time. Steps 5a and 5b close it: an over-estimated
maintenance surfaces as a stall, the stall check suggests a break, and a break *is*
Maintain — where the refusal lifts and the loop converges normally. **The correction is
deferred to the phase where acting on it is safe, not discarded.** That only became true
once 02 and 03 existed, which is why this piece was built last.

Two alternatives were **rejected**: refusing only on an outright *gain* (the downward walk
still happens on a plain stall, which is the commoner case), and *damping* the step rather
than blocking it (a new constant with no evidence behind it, and a rule that cannot be said
in one sentence). The rule as built needs **no new policy constant**.

Two cards carry it on screen. *"Weight up while eating less than maintenance"* explains the
...
Founder's call, taken once Steps 1–5 were all built. Recorded here so it is a decision with
...
**Why it is defensible to drop.** 05 was designed when it was the *only* thing standing
between a user and an open-ended deficit. That is no longer the shape of the app. Everything
underneath it now works without asking the user anything:
...
| Genuinely low fuel on a training day | The low-fuel warning (Step 4) |

The one thing 05 uniquely owned — *"if you're feeling run down with it, it's worth talking to
a doctor"* — is already in the hard break prompt's copy.

**What is genuinely lost, stated plainly.** The app can no longer catch the person whose
numbers look fine but who feels terrible. Nothing structural sees that; it needs data the app
does not hold and would have to ask for. That is a real gap, accepted knowingly.
...
6. **`LEAN_BF = 15% M / 23% F`** (Step 4) — the rule deciding who sees the low-fuel warning. Informed
