...
const dropDuplicateTotalRow = items => items.filter((it, i) => {
  const rest = items.filter((_, j) => j !== i);
  if (rest.length < 2) return true;
  const sum  = k => rest.reduce((a, o) => a + (o[k] || 0), 0);
  const near = (v, t) => Math.abs((v || 0) - t) <= Math.max(1, t * 0.05);
  const sk = sum("kcal");
...
function AILog({ onAdd, onBack }) {
  ...
  const estimate = async () => {
    ...
    try {
      ...
      const merged = dropDuplicateTotalRow(aiItems.map(it => ({ ...it, confidence: normConf(it.confidence) })));
